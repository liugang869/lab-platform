...
def checks (aim_dir, post_dir):
    ...
    print '----------------------------------'
    os.system('rm '+aim_dir+'*')
    return aim,total

if __name__ == '__main__':
    os.system('cp '+fpath+'/reference/runaimdb_s.cc '+fpath+'/../AIMDB/system/runaimdb.cc')
    os.chdir (fpath+'/../AIMDB/')
    os.system('make')
    os.chdir (fpath+'/../Lab3_Test1/')
    os.system(fpath+'/../AIMDB/runaimdb '+fpath+'/data/tpch_schema_small_r.txt '+fpath+'/data/sf10Mt/')
    aim,total = checks (fpath+'/result/aimdb_result/',fpath+'/result/post_result/')
    print 'Pass :',aim
    print 'Total:',total
    pass
